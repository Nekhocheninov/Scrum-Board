...
function Scrum() {
  getTasks(0)
    .then(tasksArray => {
      console.log('Массив задач пользователя:', tasksArray);
      lists = ['list1', 'list2', 'list3']
      const node = document.getElementById('list1');
      for (let element of tasksArray) {
        var clone = node.querySelector('li').cloneNode(true);
        clone.style = '';
        clone.insertBefore(document.createTextNode(element['title']), clone.firstChild);
        clone.querySelector('.DateTime').appendChild(document.createTextNode(element['datetime']));
        node.appendChild(clone);
      }
    })
    .catch(error => {
